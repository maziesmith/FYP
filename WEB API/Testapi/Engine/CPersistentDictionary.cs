...
namespace Testapi.Engine
{
    public class CPersistentDictionary : IDisposable
    {
        ...
        string _completeFilename = @"C:\persist.dic";//index file will be, persist.hash.idx
        string _indexFile = string.Empty;
        int _arrayOffset = 0;//always zero
        ...
        public void Add(string key, string value)
        {
            ...
            if (_lstIndexes.ContainsKey(key))
            {
                ...
                //Update the value
                Int64 pos = Convert.ToInt64(_lstIndexes[key].Split(new char[] { '|' })[0]);
                int len = Convert.ToInt32(_lstIndexes[key].Split(new char[] { '|' })[1]);
        ...
        private string Read(string key)
        {
            ...
            try
            {
                ...
                p = _lstIndexes[key].Split(new char[] { '|' })[0];
                l = _lstIndexes[key].Split(new char[] { '|' })[1];
                Int64 pos = Convert.ToInt64(p);
